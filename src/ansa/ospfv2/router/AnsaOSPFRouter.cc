 ...
 */
AnsaOSPF::Router::Router(AnsaOSPF::RouterID id, cSimpleModule* containingModule) :
    routerID(id),
...
AnsaOSPF::Interface* AnsaOSPF::Router::GetNonVirtualInterface(unsigned char ifIndex)
{
    long areaCount = areas.size();
    for (long i = 0; i < areaCount; i++) {
        AnsaOSPF::Interface* intf = areas[i]->GetInterface(ifIndex);
        if (intf != NULL) {
            return intf;
        }
    }
 ...
 */
bool AnsaOSPF::Router::InstallLSA(OSPFLSA* lsa, AnsaOSPF::AreaID areaID /*= BackboneAreaID*/)
{
 ...
 * @return True if the routing table needs to be updated, false otherwise.
 */
bool AnsaOSPF::Router::InstallASExternalLSA(OSPFASExternalLSA* lsa)
{
    /**
 ...
 */
OSPFLSA* AnsaOSPF::Router::FindLSA(LSAType lsaType, AnsaOSPF::LSAKeyType lsaKey, AnsaOSPF::AreaID areaID)
{
    switch (lsaType) {
        case RouterLSAType:
            {
 ...
 */
AnsaOSPF::ASExternalLSA* AnsaOSPF::Router::FindASExternalLSA(AnsaOSPF::LSAKeyType lsaKey)
{
    std::map<AnsaOSPF::LSAKeyType, AnsaOSPF::ASExternalLSA*, AnsaOSPF::LSAKeyType_Less>::iterator lsaIt = asExternalLSAsByID.find(lsaKey);
    if (lsaIt != asExternalLSAsByID.end()) {
        return lsaIt->second;
 ...
 */
const AnsaOSPF::ASExternalLSA* AnsaOSPF::Router::FindASExternalLSA(AnsaOSPF::LSAKeyType lsaKey) const
{
 ...
 */
void AnsaOSPF::Router::RemoveFromAllRetransmissionLists(AnsaOSPF::LSAKeyType lsaKey)
{
    long areaCount = areas.size();
    for (long i = 0; i < areaCount; i++) {
        areas[i]->RemoveFromAllRetransmissionLists(lsaKey);
 ...
 */
bool AnsaOSPF::Router::FloodLSA(OSPFLSA* lsa, AnsaOSPF::AreaID areaID /*= BackboneAreaID*/, AnsaOSPF::Interface* intf /*= NULL*/, AnsaOSPF::Neighbor* neighbor /*= NULL*/)
{
 ...
 */
AnsaOSPF::ASExternalLSA* AnsaOSPF::Router::OriginateASExternalLSA(AnsaOSPF::ASExternalLSA* lsa)
{
 ...
 */
AnsaOSPF::RoutingTableEntry* AnsaOSPF::Router::Lookup(IPAddress destination, std::vector<AnsaOSPF::RoutingTableEntry*>* table /*= NULL*/) const
{
    const std::vector<AnsaOSPF::RoutingTableEntry*>& rTable           = (table == NULL) ? routingTable : (*table);
    unsigned long                                dest             = destination.getInt();
    unsigned long                                routingTableSize = rTable.size();
    bool                                         unreachable      = false;
    std::vector<AnsaOSPF::RoutingTableEntry*>        discard;
    unsigned long                                i;
    ...
    for (i = 0; i < areaCount; i++) {
        ...
        for (unsigned int j = 0; j < addressRangeCount; j++) {
            AnsaOSPF::IPv4AddressRange range = areas[i]->GetAddressRange(j);

            for (unsigned int k = 0; k < routingTableSize; k++) {
                AnsaOSPF::RoutingTableEntry* entry = rTable[k];
                ...
                if (((entry->GetDestinationID().getInt() & entry->GetAddressMask().getInt() & ULongFromIPv4Address(range.mask)) == ULongFromIPv4Address(range.address & range.mask)) &&
                    (entry->GetPathType() == AnsaOSPF::RoutingTableEntry::IntraArea))
                {
                    // active area address range
                    AnsaOSPF::RoutingTableEntry* discardEntry = new AnsaOSPF::RoutingTableEntry;
                    ...
                    discardEntry->SetDestinationType(AnsaOSPF::RoutingTableEntry::NetworkDestination);
                    discardEntry->SetPathType(AnsaOSPF::RoutingTableEntry::InterArea);
    ...
    AnsaOSPF::RoutingTableEntry* bestMatch = NULL;
    unsigned long            longestMatch = 0;

    for (i = 0; i < routingTableSize; i++) {
        if (rTable[i]->GetDestinationType() == AnsaOSPF::RoutingTableEntry::NetworkDestination) {
            AnsaOSPF::RoutingTableEntry* entry        = rTable[i];
    ...
    if (bestMatch == NULL) {
        ...
        for (i = 0; i < discardCount; i++) {
            AnsaOSPF::RoutingTableEntry* entry        = discard[i];
            unsigned long            entryAddress = entry->GetDestinationID().getInt();
            unsigned long            entryMask    = entry->GetAddressMask().getInt();
 ...
 */
bool AnsaOSPF::Router::HasRouteToASBoundaryRouter(const std::vector<AnsaOSPF::RoutingTableEntry*>& inRoutingTable, AnsaOSPF::RouterID asbrRouterID) const
{
    long routeCount = inRoutingTable.size();
    for (long i = 0; i < routeCount; i++) {
        AnsaOSPF::RoutingTableEntry* routingEntry = inRoutingTable[i];
        if (((routingEntry->GetDestinationType() & AnsaOSPF::RoutingTableEntry::ASBoundaryRouterDestination) != 0) &&
 ...
 * @param asbrRouterID     [in] The ID of the AS Boundary Router to look for.
 */
std::vector<AnsaOSPF::RoutingTableEntry*> AnsaOSPF::Router::GetRoutesToASBoundaryRouter(const std::vector<AnsaOSPF::RoutingTableEntry*>& fromRoutingTable, AnsaOSPF::RouterID asbrRouterID) const
{
    std::vector<AnsaOSPF::RoutingTableEntry*> results;
    long                                  routeCount = fromRoutingTable.size();

    for (long i = 0; i < routeCount; i++) {
        AnsaOSPF::RoutingTableEntry* routingEntry = fromRoutingTable[i];
        if (((routingEntry->GetDestinationType() & AnsaOSPF::RoutingTableEntry::ASBoundaryRouterDestination) != 0) &&
            (routingEntry->GetDestinationID().getInt() == asbrRouterID))
        {
 ...
 */
void AnsaOSPF::Router::PruneASBoundaryRouterEntries(std::vector<AnsaOSPF::RoutingTableEntry*>& asbrEntries) const
{
    bool hasNonBackboneIntraAreaPath = false;
    for (std::vector<AnsaOSPF::RoutingTableEntry*>::iterator it = asbrEntries.begin(); it != asbrEntries.end(); it++) {
        AnsaOSPF::RoutingTableEntry* routingEntry = *it;
    ...
    if (hasNonBackboneIntraAreaPath) {
        std::vector<AnsaOSPF::RoutingTableEntry*>::iterator it = asbrEntries.begin();
        while (it != asbrEntries.end()) {
            if (((*it)->GetPathType() != AnsaOSPF::RoutingTableEntry::IntraArea) ||
                ((*it)->GetArea() == AnsaOSPF::BackboneAreaID))
            {
 ...
 */
AnsaOSPF::RoutingTableEntry* AnsaOSPF::Router::SelectLeastCostRoutingEntry(std::vector<AnsaOSPF::RoutingTableEntry*>& entries) const
{
    if (entries.empty()) {
        return NULL;
    }

    AnsaOSPF::RoutingTableEntry* leastCostEntry = entries[0];
 ...
 * @sa AnsaOSPF::Area::OriginateSummaryLSA
 */
AnsaOSPF::RoutingTableEntry* AnsaOSPF::Router::GetPreferredEntry(const OSPFLSA& lsa, bool skipSelfOriginated, std::vector<AnsaOSPF::RoutingTableEntry*>* fromRoutingTable /*= NULL*/)
{
    ...
    AnsaOSPF::RouterID           originatingRouter = lsaHeader.getAdvertisingRouter().getInt();
    ...
    if (forwardingAddress.isUnspecified()) {   // (3)
        std::vector<AnsaOSPF::RoutingTableEntry*> asbrEntries = GetRoutesToASBoundaryRouter(*fromRoutingTable, originatingRouter);
        if (!rfc1583Compatibility) {
            PruneASBoundaryRouterEntries(asbrEntries);
        }
        return SelectLeastCostRoutingEntry(asbrEntries);
    } else {
        AnsaOSPF::RoutingTableEntry* forwardEntry = Lookup(forwardingAddress, fromRoutingTable);

        if (forwardEntry == NULL) {
            return NULL;
        }

        if ((forwardEntry->GetPathType() != AnsaOSPF::RoutingTableEntry::IntraArea) &&
            (forwardEntry->GetPathType() != AnsaOSPF::RoutingTableEntry::InterArea))
        {
            return NULL;
        }
 ...
 */
void AnsaOSPF::Router::CalculateASExternalRoutes(std::vector<AnsaOSPF::RoutingTableEntry*>& newRoutingTable)
{
    unsigned long lsaCount = asExternalLSAs.size();
    unsigned long i;

    for (i = 0; i < lsaCount; i++) {
        AnsaOSPF::ASExternalLSA* currentLSA        = asExternalLSAs[i];
        OSPFLSAHeader&       currentHeader     = currentLSA->getHeader();
        unsigned short       externalCost      = currentLSA->getContents().getRouteCost();
        AnsaOSPF::RouterID       originatingRouter = currentHeader.getAdvertisingRouter().getInt();

        AnsaOSPF::RoutingTableEntry* preferredEntry = GetPreferredEntry(*currentLSA, true, &newRoutingTable);
        ...
        AnsaOSPF::RoutingTableEntry* destinationEntry = Lookup(destination, &newRoutingTable);   // (5)
        if (destinationEntry == NULL) {
            ...
            newEntry->SetPathType(type2ExternalMetric ? AnsaOSPF::RoutingTableEntry::Type2External : AnsaOSPF::RoutingTableEntry::Type1External);
            ...
            AnsaOSPF::RoutingTableEntry::RoutingPathType destinationPathType = destinationEntry->GetPathType();
            bool                                     type2ExternalMetric = currentLSA->getContents().getE_ExternalMetricType();
            unsigned int                             nextHopCount        = preferredEntry->GetNextHopCount();

            if ((destinationPathType == AnsaOSPF::RoutingTableEntry::IntraArea) ||
                (destinationPathType == AnsaOSPF::RoutingTableEntry::InterArea))   // (6) (a)
            {
                continue;
            }

            if (((destinationPathType == AnsaOSPF::RoutingTableEntry::Type1External) &&
                 (type2ExternalMetric)) ||
                ((destinationPathType == AnsaOSPF::RoutingTableEntry::Type2External) &&
                 (type2ExternalMetric) &&
            ...
            AnsaOSPF::RoutingTableEntry* destinationPreferredEntry = GetPreferredEntry(*(destinationEntry->GetLinkStateOrigin()), false, &newRoutingTable);
            if ((!rfc1583Compatibility) &&
                (destinationPreferredEntry->GetPathType() == AnsaOSPF::RoutingTableEntry::IntraArea) &&
                (destinationPreferredEntry->GetArea() != AnsaOSPF::BackboneAreaID) &&
                ((preferredEntry->GetPathType() != AnsaOSPF::RoutingTableEntry::IntraArea) ||
                 (preferredEntry->GetArea() == AnsaOSPF::BackboneAreaID)))
            {
                continue;
            }
            ...
            destinationEntry->SetPathType(type2ExternalMetric ? AnsaOSPF::RoutingTableEntry::Type2External : AnsaOSPF::RoutingTableEntry::Type1External);
            if (type2ExternalMetric) {
            ...
            destinationEntry->SetDestinationType(AnsaOSPF::RoutingTableEntry::NetworkDestination);
 ...
 *         AnsaOSPF::NullIPv4AddressRange otherwise.
 */
AnsaOSPF::IPv4AddressRange AnsaOSPF::Router::GetContainingAddressRange(AnsaOSPF::IPv4AddressRange addressRange, bool* advertise /*= NULL*/) const
{
    unsigned long areaCount = areas.size();
    for (unsigned long i = 0; i < areaCount; i++) {
 ...
 * @sa AnsaOSPF::Area::GetUniqueLinkStateID
 */
...
void AnsaOSPF::Router::NotifyAboutRoutingTableChanges(std::vector<AnsaOSPF::RoutingTableEntry*>& oldRoutingTable)
{
    if (areas.size() <= 1) {
        return;
    }
    ...
    unsigned long areaCount = areas.size();
    for (i = 0; i < areaCount; i++) {
        std::map<AnsaOSPF::LSAKeyType, bool, AnsaOSPF::LSAKeyType_Less> originatedLSAMap;
        std::map<AnsaOSPF::LSAKeyType, bool, AnsaOSPF::LSAKeyType_Less> deletedLSAMap;
        AnsaOSPF::LSAKeyType                                        lsaKey;
        ...
        for (j = 0; j < routeCount; j++) {
            ...
            if (destIt == oldTableMap.end()) { // new routing entry
                AnsaOSPF::SummaryLSA* lsaToReoriginate = NULL;
                AnsaOSPF::SummaryLSA* newLSA           = areas[i]->OriginateSummaryLSA(routingTable[j], originatedLSAMap, lsaToReoriginate);

                if (newLSA != NULL) {
                    if (lsaToReoriginate != NULL) {
                        areas[i]->InstallSummaryLSA(lsaToReoriginate);
//                        FloodLSA(lsaToReoriginate, AnsaOSPF::BackboneAreaID);
...
//                    FloodLSA(newLSA, AnsaOSPF::BackboneAreaID);
...
//                        FloodLSA(newLSA, AnsaOSPF::BackboneAreaID);
                        ...
                        if ((routingTable[j]->GetDestinationType() == AnsaOSPF::RoutingTableEntry::NetworkDestination) &&
                            ((routingTable[j]->GetPathType() == AnsaOSPF::RoutingTableEntry::IntraArea) ||
                             (routingTable[j]->GetPathType() == AnsaOSPF::RoutingTableEntry::InterArea)))
                        {
                            AnsaOSPF::IPv4AddressRange containingAddressRange = GetContainingAddressRange(destinationAddressRange);
                            if (containingAddressRange != AnsaOSPF::NullIPv4AddressRange) {
                        ...
                        for (k = 0; k < routeCount; k++) {
                            if ((routingTable[k]->GetDestinationType() == AnsaOSPF::RoutingTableEntry::NetworkDestination) &&
                                (routingTable[k]->GetPathType() == AnsaOSPF::RoutingTableEntry::IntraArea) &&
                        ...
                        if (maxRangeCost == routingTable[j]->GetCost()) {  // this entry gives the range's cost
                            ...
                            AnsaOSPF::SummaryLSA* summaryLSA = areas[i]->FindSummaryLSA(lsaKey);

                            if (summaryLSA != NULL) {
                                if (oneLessCost != 0) { // there's an other entry in this range
                                    summaryLSA->setRouteCost(oneLessCost);
//                                    FloodLSA(summaryLSA, AnsaOSPF::BackboneAreaID);
                                    FloodLSA(summaryLSA, areas[i]->GetAreaID());

                                    originatedLSAMap[lsaKey] = true;
                                } else {    // no more entries in this range -> delete it
                                    std::map<AnsaOSPF::LSAKeyType, bool, AnsaOSPF::LSAKeyType_Less>::const_iterator deletedIt = deletedLSAMap.find(lsaKey);
...
//                                        FloodLSA(summaryLSA, AnsaOSPF::BackboneAreaID);
        ...
        for (j = 0; j < routeCount; j++) {
            ...
            if (destIt == newTableMap.end()) { // deleted routing entry
                ...
                if ((oldRoutingTable[j]->GetDestinationType() == AnsaOSPF::RoutingTableEntry::NetworkDestination) &&
                    ((oldRoutingTable[j]->GetPathType() == AnsaOSPF::RoutingTableEntry::IntraArea) ||
                     (oldRoutingTable[j]->GetPathType() == AnsaOSPF::RoutingTableEntry::InterArea)))
                {
                    AnsaOSPF::IPv4AddressRange containingAddressRange = GetContainingAddressRange(destinationAddressRange);
                    if (containingAddressRange != AnsaOSPF::NullIPv4AddressRange) {
                        destinationAddressRange = containingAddressRange;
 ...
 * @param ifIndex               [in]
 */
void AnsaOSPF::Router::UpdateExternalRoute(AnsaOSPF::IPv4Address networkAddress, const OSPFASExternalLSAContents& externalRouteContents, int ifIndex)
{
