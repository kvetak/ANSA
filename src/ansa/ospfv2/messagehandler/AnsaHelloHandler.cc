...
AnsaOSPF::HelloHandler::HelloHandler(AnsaOSPF::Router* containingRouter) :
    AnsaOSPF::IMessageHandler(containingRouter)
{
}

void AnsaOSPF::HelloHandler::ProcessPacket(OSPFPacket* packet, AnsaOSPF::Interface* intf, AnsaOSPF::Neighbor* unused)
{
    ...
    {
        AnsaOSPF::Interface::OSPFInterfaceType interfaceType = intf->GetType();
           ...
           )
        {
            ...
            if (intf->GetArea()->GetExternalRoutingCapability() == helloPacket->getOptions().E_ExternalRoutingCapability) {
                ...
                AnsaOSPF::IPv4Address   srcAddress              = IPv4AddressFromULong(controlInfo->getSrcAddr().getInt());
                ...
                if ((interfaceType == AnsaOSPF::Interface::Broadcast) ||
                    ...
                    (interfaceType == AnsaOSPF::Interface::NBMA))
                {
                ...
                if (neighbor != NULL) {
                    ...
                    AnsaOSPF::DesignatedRouterID    dRouterID;

                    if ((interfaceType == AnsaOSPF::Interface::Virtual) &&
                        (neighbor->GetState() == AnsaOSPF::Neighbor::DownState))
                    {
                    ...
                    if ((newDesignatedRouter == source) &&
                        (newBackupRouter == 0) &&
                        (intf->GetState() == AnsaOSPF::Interface::WaitingState))
                    {
                        backupSeen = true;
                    } else {
                    ...
                    if (!neighbor->DesignatedRoutersAreSetUp()) {
                        AnsaOSPF::Neighbor* designated = intf->GetNeighborByAddress(designatedAddress);
                        AnsaOSPF::Neighbor* backup     = intf->GetNeighborByAddress(backupAddress);
                    ...
                    neighbor = new AnsaOSPF::Neighbor(helloPacket->getRouterID().getInt());
                    neighbor->SetPriority(helloPacket->getRouterPriority());
                    neighbor->SetAddress(srcAddress);
                    neighbor->SetRouterDeadInterval(helloPacket->getRouterDeadInterval());
                    ...
                    AnsaOSPF::Neighbor* designated = intf->GetNeighborByAddress(dRouterID.ipInterfaceAddress);
                    ...
                    AnsaOSPF::Neighbor* backup = intf->GetNeighborByAddress(dRouterID.ipInterfaceAddress);
                ...
                neighbor->ProcessEvent(AnsaOSPF::Neighbor::HelloReceived);
                if ((interfaceType == AnsaOSPF::Interface::NBMA) &&
                    (intf->GetRouterPriority() == 0) &&
                    (neighbor->GetState() >= AnsaOSPF::Neighbor::InitState))
                {
                ...
                if (i == neighborsNeighborCount) {
                    neighbor->ProcessEvent(AnsaOSPF::Neighbor::OneWayReceived);
                }

                if (neighborChanged) {
                    intf->ProcessEvent(AnsaOSPF::Interface::NeighborChange);
                    ...
                    for (i = 0; i < neighborCount; i++) {
                        AnsaOSPF::Neighbor* stuckNeighbor = intf->GetNeighbor(i);
                        if (stuckNeighbor->GetState() == AnsaOSPF::Neighbor::TwoWayState) {
                            stuckNeighbor->ProcessEvent(AnsaOSPF::Neighbor::IsAdjacencyOK);
                        }
                    }

                    if (neighborsDRStateChanged) {
                        AnsaOSPF::RouterLSA* routerLSA = intf->GetArea()->FindRouterLSA(router->GetRouterID());
                ...
                if (backupSeen) {
                    intf->ProcessEvent(AnsaOSPF::Interface::BackupSeen);
                }
            }
        }
    }
