...
#include "ansa/linklayer/ethernet/switch/ANSA_MACRelayUnit.h"
...
Define_Module(ANSA_MACRelayUnit);

void ANSA_MACRelayUnit::initialize(int stage)
{
...
void ANSA_MACRelayUnit::handleMessage(cMessage *msg)
{
    if (!isOperational) {
...
void ANSA_MACRelayUnit::handleAndDispatchFrame(EtherFrame *frame)
{
...
void ANSA_MACRelayUnit::broadcastFrame(EtherFrame *frame, int inputport)
{
...
void ANSA_MACRelayUnit::start()
{
    addressTable->clearTable();
    isOperational = true;
}

void ANSA_MACRelayUnit::stop()
{
    addressTable->clearTable();
    isOperational = false;
}

bool ANSA_MACRelayUnit::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
