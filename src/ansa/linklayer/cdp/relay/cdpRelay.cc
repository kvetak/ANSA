...
#include "ansa/linklayer/cdp/relay/cdpRelay.h"
...
Define_Module(cdpRelay);

cdpRelay::cdpRelay()
{
}

void cdpRelay::initialize(int stage)
{
    if (stage == INITSTAGE_LOCAL) {
        ...
        isCdpAware = true;    // if the stpIn is not connected then the switch is STP/RSTP unaware
...
void cdpRelay::handleMessage(cMessage *msg)
{
    if (!isOperational) {
...
void cdpRelay::broadcast(EtherFrame *frame)
{
...
void cdpRelay::handleAndDispatchFrame(EtherFrame *frame)
{
    ...
    if ( frame->getDest() == bridgeGroupCDPAddress) {
        EV_DETAIL << "Deliver BPDU to the CDP module" << endl;
        deliverCDPUpdate(frame);    // deliver to the STP/RSTP module
    }
    else if (!isCdpAware) {
...
void cdpRelay::dispatch(EtherFrame *frame, unsigned int portNum)
{
...
void cdpRelay::learn(EtherFrame *frame)
{
    int arrivalGate = frame->getArrivalGate()->getIndex();
...
void cdpRelay::dispatchCDPUpdate(CDPUpdate *cdpUpdate, int port)
{
    if (port >= portCount)
...
void cdpRelay::deliverCDPUpdate(EtherFrame *frame)
{
...
InterfaceEntry *cdpRelay::chooseInterface()
{
...
bool cdpRelay::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
