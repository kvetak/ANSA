...
{
    if(dynamic_cast<CDPOptionBase *> (opt))
    {
        CDPOptionBase *option = dynamic_cast<CDPOptionBase *> (opt);
        option->setLength(strlen(option->getValue()) + TL_SIZE);
    }
    else if(dynamic_cast<CDPOptionCapa *> (opt))
    {
        CDPOptionCapa *option = dynamic_cast<CDPOptionCapa *> (opt);
        option->setLength(option->getCapArraySize() + TL_SIZE);
    }
    else if(dynamic_cast<CDPOptionDupl *> (opt))
    {
        CDPOptionDupl *option = dynamic_cast<CDPOptionDupl *> (opt);
        option->setLength(sizeof(option->getFullDuplex()) + TL_SIZE);
    }
    else if(dynamic_cast<CDPOptionAddr *> (opt))
    {
        ...
        option->setLength(size + TL_SIZE);
    }
    else if(dynamic_cast<CDPOptionPref *> (opt))
    {
        CDPOptionPref *option = dynamic_cast<CDPOptionPref *> (opt);
        option->setLength(option->getPrefixesArraySize()*5 + TL_SIZE);
    }
}

std::string CDPUpdate::getOptionStr(TLVOptionBase *opt)
{
    std::string s;
