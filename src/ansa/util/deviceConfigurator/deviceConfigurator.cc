...
void DeviceConfigurator::loadBabelConfig(BabelMain *bMain)
{
    ASSERT(bMain != NULL);
...
void DeviceConfigurator::loadBabelProcessConfig(cXMLElement *device, BabelMain *bMain)
{
    ASSERT(bMain != NULL);
    ASSERT(device != NULL);
    ...
    processElem = xmlParser::GetBabelProcess(device);
    if (processElem == NULL)
    ...
    {
        int porttmp;
        bool success = xmlParser::Str2Int(&porttmp, portElem->getNodeValue());
...
void DeviceConfigurator::loadBabelInterfacesConfig(cXMLElement *device, BabelMain *bMain)
{
    cXMLElement *ifaceElem = NULL;
    ...
    while (ifaceElem != NULL)
    {
        ...
        {// interface contains babel configuration
            ...
            // for each IPv6 address - save info about network prefix
            cXMLElement *ipv6AddrElem = xmlParser::GetIPv6Address(NULL, ifaceElem);
            while (ipv6AddrElem != NULL)
            {
                // get address string
                string addrFull = ipv6AddrElem->getNodeValue();
...
void DeviceConfigurator::loadBabelInterface(cXMLElement *ifaceElem, BabelMain *bMain, BabelInterface *bIface)
{
    int tempNumber;
    bool tempBool, success;
