...
void RIPDeviceConfigurator::loadRIPConfig(RIPRouting *RIPModule)
{
    ...
    //RIP NETWORK COMMAND
    networkElement = GetRIPNetwork(NULL, device);
    ...
    {// process all RIP Network "command"
        ...
        networkElement = GetRIPNetwork(networkElement, NULL);
    }

    //PASSIVE-INTERFACE
    cXMLElement *passiveInterfaceElem;
    passiveInterfaceElem = GetRIPPassiveInterface(NULL, device);
    while(passiveInterfaceElem != NULL)
    {
        ...
        passiveInterfaceElem = GetRIPPassiveInterface(passiveInterfaceElem, NULL);
    }
    ...
    interfaceElem = GetInterface(NULL, device);
    while (interfaceElem != NULL)
    {
        interfaceName = interfaceElem->getAttribute("name");
        RIPInterfaceSplitHorizon = GetRIPInterfaceSplitHorizon(interfaceElem);
        ...
        if (RIPInterface != NULL)
        {
            ...
            RIPInterfacePoisonReverse = GetRIPInterfacePoisonReverse(interfaceElem);
        ...
        //next interface
        interfaceElem = GetInterface(interfaceElem, NULL);
    }
}

void RIPDeviceConfigurator::loadNetworksFromInterfaceToRIPRT(RIPRouting *RIPModule, InterfaceEntry *interface)
{
...
cXMLElement * RIPDeviceConfigurator::GetInterface(cXMLElement *iface, cXMLElement *device){
...
cXMLElement *RIPDeviceConfigurator::GetRIPPassiveInterface(cXMLElement *passiveInterface, cXMLElement *device)
{
    // initial call of the method - find <Routing> -> <RIP>
...
const char *RIPDeviceConfigurator::GetRIPInterfaceSplitHorizon(cXMLElement *iface)
{
    return GetNodeParamConfig(iface, "RIPSplitHorizon", "enable");
}

const char *RIPDeviceConfigurator::GetRIPInterfacePoisonReverse(cXMLElement *iface)
{
