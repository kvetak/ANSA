...
#include "ansa/networklayer/eigrp/pdms/EigrpIpv6Pdm.h"
...
EigrpIpv6Pdm::EigrpIpv6Pdm() : EIGRP_IPV6_MULT(inet::IPv6Address("FF02::A"))
{
...
void EigrpIpv6Pdm::initialize(int stage)
{
    ...
    if (stage == 3)
    {
        ...
        //this->eigrpDual = inet::ModuleAccess<EigrpDual>("eigrpDual").get();

        this->ift = InterfaceTableAccess().get();
        this->nb = NotificationBoardAccess().get();

        this->eigrpIftDisabled = new EigrpDisabledInterfaces();
        this->routingForNetworks = new EigrpNetworkTable<inet::IPv6Address>();
        this->eigrpDual = new EigrpDual<inet::IPv6Address>(this);
...
void EigrpIpv6Pdm::receiveChangeNotification(int category, const cObject *details)
{
    ...
    else if (category == inet::NF_IPv6_ROUTE_DELETED)
    { //
        processRTRouteDel(details);
    }
}

void EigrpIpv6Pdm::processIfaceStateChange(inet::InterfaceEntry *iface)
{
    ...
    { // an interface goes up
        ...
        {// through all known prefixes search prefixes belong to this interface
            ...
            {// belonging to same interface -> add
                if(rt->findRoute(it->network, it->prefixLength, inet::IPv6Address::UNSPECIFIED_ADDRESS) == NULL)
                {// route is not in RT -> add
                    rt->addStaticRoute(it->network, it->prefixLength, ifaceId, inet::IPv6Address::UNSPECIFIED_ADDRESS, 0);
                }
            }
        }
    ...
    { // an interface goes down
        ...
        for(int i = 0; i < rt->getNumRoutes(); ++i)
        {
            ...
            if(route->getInterface()->getInterfaceId() == ifaceId && route->getNextHop() == inet::IPv6Address::UNSPECIFIED_ADDRESS && route->getDestPrefix() != inet::IPv6Address::LINKLOCAL_PREFIX)
...
{
    EigrpRouteSource<inet::IPv6Address> *source;
    ...
    for (int i = 0; i < routeCount; i++)
    {
        ...
        if (source->getIfaceId() == ifaceId && source->isValid())
        {
            ...
            if (source->getNexthopId() == EigrpNeighbor<inet::IPv6Address>::UNSPEC_ID)
            { // connected route
            ...
            eigrpDual->processEvent(EigrpDual<inet::IPv6Address>::RECV_UPDATE, source, source->getNexthopId(), false);
        }
    }
...
{
    inet::IPv6Route *changedRt = check_and_cast<inet::IPv6Route *>(details);
    ANSAIPv6Route *changedAnsaRt = dynamic_cast<ANSAIPv6Route *>(changedRt);
    unsigned adminDist;
    EigrpRouteSource<inet::IPv6Address> *source = NULL;
    ...
    { // Deletion of EIGRP internal route
        ...
        if (source->isSuccessor())
        {
            ...
            { // Process route in DUAL (no change of metric)
                this->eigrpDual->processEvent(EigrpDual<inet::IPv6Address>::LOST_ROUTE, source, IEigrpPdm::UNSPEC_SENDER, false);
            }
        }
...
void EigrpIpv6Pdm::processMsgFromNetwork(cMessage *msg)
{
    ...
    inet::InterfaceEntry::State status = ift->getInterfaceById(ifaceId)->getState();
    if(status == inet::InterfaceEntry::DOWN || status == inet::InterfaceEntry::GOING_UP )
    ...
    EigrpNeighbor<inet::IPv6Address> *neigh;
...
bool EigrpIpv6Pdm::getDestIpAddress(int destNeigh, inet::IPv6Address *resultAddress)
{
    EigrpNeighbor<inet::IPv6Address> *neigh = NULL;
    const uint32 *addr = NULL;

    if (destNeigh == EigrpNeighbor<inet::IPv6Address>::UNSPEC_ID)
...
void EigrpIpv6Pdm::printSentMsg(int routeCnt, inet::IPv6Address& destAddress, EigrpMsgReq *msgReq)
{
...
void EigrpIpv6Pdm::printRecvMsg(EigrpMessage *msg, inet::IPv6Address& addr, int ifaceId)
{
...
void EigrpIpv6Pdm::processTimer(cMessage *msg)
{
    ...
    case EIGRP_HELLO_TIMER:
        ...
        // send Hello message
        msgReq = createMsgReq(EIGRP_HELLO_MSG, EigrpNeighbor<inet::IPv6Address>::UNSPEC_ID, eigrpIface->getInterfaceId());
    ...
    case EIGRP_HOLD_TIMER:
        ...
        contextBasePtr = (cObject*)timer->getContextPointer();
        neigh = check_and_cast<EigrpNeighbor<inet::IPv6Address> *>(contextBasePtr);
        ...
        msgReq = createMsgReq(EIGRP_HELLO_MSG, EigrpNeighbor<inet::IPv6Address>::UNSPEC_ID, ifaceId);
...
void EigrpIpv6Pdm::processAckMsg(cMessage *msg, inet::IPv6Address& srcAddress, int ifaceId, EigrpNeighbor<inet::IPv6Address> *neigh)
{
...
void EigrpIpv6Pdm::processHelloMsg(cMessage *msg, inet::IPv6Address& srcAddress, int ifaceId, EigrpNeighbor<inet::IPv6Address> *neigh)
{
    ...
    { // neighbor exists, its state is "up" or "pending"
        ...
        { // Not satisfied
            ...
            EigrpMsgReq *msgReq = createMsgReq(EIGRP_HELLO_MSG, EigrpNeighbor<inet::IPv6Address>::UNSPEC_ID, ifaceId);
            msgReq->setGoodbyeMsg(true);
            send(msgReq, RTP_OUTGW);
        }
...
void EigrpIpv6Pdm::processUpdateMsg(cMessage *msg, inet::IPv6Address& srcAddress, int ifaceId, EigrpNeighbor<inet::IPv6Address> *neigh)
{
    EigrpIpv6Update *update = check_and_cast<EigrpIpv6Update *>(msg);
    EigrpInterface *eigrpIface = eigrpIft->findInterfaceById(ifaceId);
    EigrpRouteSource<inet::IPv6Address> *src;
    ...
    { // Neighbor is "up", forward message to DUAL
        ...
        for (int i = 0; i < cnt; i++)
        {
            ...
            { // Route with RID is equal to RID of router or tlv is unreachable route
                inet::IPv6Address nextHop = getNextHopAddr(tlv.nextHop, srcAddress);
            ...
            { // process route
                ...
                if (notifyDual)
                    eigrpDual->processEvent(EigrpDual<inet::IPv6Address>::RECV_UPDATE, src, neigh->getNeighborId(), isSourceNew);
                else
...
void EigrpIpv6Pdm::processQueryMsg(cMessage *msg, inet::IPv6Address& srcAddress, int ifaceId, EigrpNeighbor<inet::IPv6Address> *neigh)
{
    EigrpIpv6Query *query = check_and_cast<EigrpIpv6Query *>(msg);
    EigrpInterface *eigrpIface = eigrpIft->findInterfaceById(ifaceId);
    EigrpRouteSource<inet::IPv6Address> *src = NULL;
    ...
    for (int i = 0; i < cnt; i++)
    {
        ...
        // Always notify DUAL
        eigrpDual->processEvent(EigrpDual<inet::IPv6Address>::RECV_QUERY, src, neigh->getNeighborId(), isSourceNew);
    }
    flushMsgRequests();
    eigrpTt->purgeTable();
}

void EigrpIpv6Pdm::processReplyMsg(cMessage *msg, inet::IPv6Address& srcAddress, int ifaceId, EigrpNeighbor<inet::IPv6Address> *neigh)
{
    ...
    for (int i = 0; i < cnt; i++)
    {
        ...
        // Always notify DUAL
        eigrpDual->processEvent(EigrpDual<inet::IPv6Address>::RECV_REPLY, src, neigh->getNeighborId(), isSourceNew);
    }
    flushMsgRequests();
    eigrpTt->purgeTable();
}
 ...
 */
EigrpRouteSource<inet::IPv6Address> *EigrpIpv6Pdm::processInterRoute(EigrpMpIpv6Internal& tlv, inet::IPv6Address& srcAddr,
        int sourceNeighId, EigrpInterface *eigrpIface, bool *notifyDual, bool *isSourceNew)
{
    inet::IPv6Address nextHop = getNextHopAddr(tlv.nextHop, srcAddr);
    EigrpNeighbor<inet::IPv6Address> *nextHopNeigh = eigrpNt->findNeighbor(nextHop);
    EigrpRouteSource<inet::IPv6Address> *src;
...
EigrpIpv6Hello *EigrpIpv6Pdm::createHelloMsg(int holdInt, EigrpKValues kValues, inet::IPv6Address& destAddress, EigrpMsgReq *msgReq)
{
...
EigrpIpv6Ack *EigrpIpv6Pdm::createAckMsg(inet::IPv6Address& destAddress, EigrpMsgReq *msgReq)
{
...
EigrpIpv6Update *EigrpIpv6Pdm::createUpdateMsg(const inet::IPv6Address& destAddress, EigrpMsgReq *msgReq)
{
...
EigrpIpv6Query *EigrpIpv6Pdm::createQueryMsg(inet::IPv6Address& destAddress, EigrpMsgReq *msgReq)
{
...
EigrpIpv6Reply *EigrpIpv6Pdm::createReplyMsg(inet::IPv6Address& destAddress, EigrpMsgReq *msgReq)
{
...
{
    inet::IPv6ControlInfo *ctrl = new inet::IPv6ControlInfo();
...
void EigrpIpv6Pdm::createRouteTlv(EigrpMpIpv6Internal *routeTlv, EigrpRoute<inet::IPv6Address> *route, bool unreachable)
{
    EigrpWideMetricPar rtMetric = route->getRdPar();
    routeTlv->destAddress = route->getRouteAddress();
    routeTlv->destMask = route->getRouteMask();
    routeTlv->nextHop = inet::IPv6Address::UNSPECIFIED_ADDRESS;
...
void EigrpIpv6Pdm::addRoutesToMsg(EigrpIpv6Message *msg, const EigrpMsgReq *msgReq)
{
    ...
    EigrpRouteSource<inet::IPv6Address> *source = NULL;
    EigrpRoute<inet::IPv6Address> *route = NULL;
...
void EigrpIpv6Pdm::sendAllEigrpPaths(EigrpInterface *eigrpIface, EigrpNeighbor<inet::IPv6Address> *neigh)
{
    int routeCnt = eigrpTt->getNumRouteInfo();
    int addedRoutes = 0;    // Number of routes in message
    EigrpRoute<inet::IPv6Address> *route;
    EigrpRouteSource<inet::IPv6Address> *source;
...
void EigrpIpv6Pdm::processNewNeighbor(int ifaceId, inet::IPv6Address &srcAddress, EigrpIpv6Hello *rcvMsg)
{
            ...
            paramTlv.kValues)) != eigrp::M_OK)
    {
        ...
        { // Send Goodbye message and reset Hello timer
            resetTimer(iface->getHelloTimer(), iface->getHelloInt() - 0.5);
            msgReqHello = createMsgReq(EIGRP_HELLO_MSG, EigrpNeighbor<inet::IPv6Address>::UNSPEC_ID, ifaceId);
...
int EigrpIpv6Pdm::checkNeighborshipRules(int ifaceId, int neighAsNum,
        inet::IPv6Address &neighAddr, const EigrpKValues &neighKValues)
{
    inet::IPv6Address ifaceAddr, ifaceMask;
    ...
    else
    {
        ...
        if (neighAddr.getScope() != inet::IPv6Address::LINK)
...
EigrpNeighbor<inet::IPv6Address> *EigrpIpv6Pdm::createNeighbor(EigrpInterface *eigrpIface, inet::IPv6Address& address, uint16_t holdInt)
{
    // Create record in the neighbor table
    EigrpNeighbor<inet::IPv6Address> *neigh = new EigrpNeighbor<inet::IPv6Address>(eigrpIface->getInterfaceId(), eigrpIface->getInterfaceName(), address);
...
void EigrpIpv6Pdm::removeNeighbor(EigrpNeighbor<inet::IPv6Address> *neigh)
{
    EigrpRouteSource<inet::IPv6Address> *source = NULL;
    EigrpRoute<inet::IPv6Address> *route = NULL;
...
#endif
                source = new EigrpRouteSource<inet::IPv6Address>(ifaceId, ifaceName, nextHopId, routeId, route);
                eigrpTt->addRoute(source);
                eigrpDual->processEvent(EigrpDual<inet::IPv6Address>::NEIGHBOR_DOWN, source, nextHopId, true);
            }
            else
                eigrpDual->processEvent(EigrpDual<inet::IPv6Address>::NEIGHBOR_DOWN, source, nextHopId, false);
        }
        else
        { // Notify DUAL about event
            if (source != NULL)
...
ANSAIPv6Route *EigrpIpv6Pdm::createRTRoute(EigrpRouteSource<inet::IPv6Address> *successor)
{
    EigrpRoute<inet::IPv6Address> *route = successor->getRouteInfo();
    ANSAIPv6Route *rtEntry = new ANSAIPv6Route(route->getRouteAddress(), getNetmaskLength(route->getRouteMask()), inet::IPv6Route::ROUTING_PROT);
...
void EigrpIpv6Pdm::msgToAllIfaces(int destination, HeaderOpcode msgType, EigrpRouteSource<inet::IPv6Address> *source, bool forcePoisonRev, bool forceUnreachable)
{
    EigrpInterface *eigrpIface;
...
void EigrpIpv6Pdm::msgToIface(HeaderOpcode msgType, EigrpRouteSource<inet::IPv6Address> *source, EigrpInterface *eigrpIface, bool forcePoisonRev, bool forceUnreachable)
{
    EigrpNeighbor<inet::IPv6Address> *neigh = NULL;
    EigrpMsgRoute msgRt;
...
bool EigrpIpv6Pdm::applySplitHorizon(EigrpInterface *destInterface, EigrpRouteSource<inet::IPv6Address> *source, EigrpRoute<inet::IPv6Address> *route)
{
...
bool EigrpIpv6Pdm::applyStubToUpdate(EigrpRouteSource<inet::IPv6Address> *src)
{
    if (this->eigrpStub.recvOnlyRt)
        return true;    // Do not send any route

    // Send only specified type of route
    else if (this->eigrpStub.connectedRt && src->getNexthopId() == EigrpNeighbor<inet::IPv6Address>::UNSPEC_ID)
        return false;
...
void EigrpIpv6Pdm::disableInterface(inet::InterfaceEntry *iface, EigrpInterface *eigrpIface)
{
    EigrpTimer* hellot;
    EigrpNeighbor<inet::IPv6Address> *neigh;
    EigrpRouteSource<inet::IPv6Address> *source;
    ...
    EigrpNetwork<inet::IPv6Address> *eigrpnet = NULL;

    for (it = eigrpIface->getNetworksIdsBegin(); it != eigrpIface->getNetworksIdsEnd(); ++it)
    {
        eigrpnet = routingForNetworks->findNetworkById(*it);
        source = eigrpTt->findRoute(eigrpnet->getAddress(), eigrpnet->getMask(), EigrpNeighbor<inet::IPv6Address>::UNSPEC_ID);
        ASSERT(source != NULL);
        // Notify DUAL about event
        eigrpDual->processEvent(EigrpDual<inet::IPv6Address>::INTERFACE_DOWN, source, EigrpNeighbor<inet::IPv6Address>::UNSPEC_ID, false);
    }
...
{
    inet::InterfaceEntry *iface = ift->getInterfaceById(ifaceId);
    ...
    {// iface not found -> create new
        eigrpIface = new EigrpInterface(iface, EigrpNetworkTable<inet::IPv6Address>::UNSPEC_NETID, false);
    }
...
void EigrpIpv6Pdm::enableInterface(EigrpInterface *eigrpIface)
{
    ...
    inet::IPv6InterfaceData *ifaceIpv6 = ift->getInterfaceById(ifaceId)->ipv6Data();
    ...
    {// through all known prefixes search prefixes belonging to enabling interface
        ...
        {// found prefix belonging to interface
            network = it->network;
            mask = inet::IPv6Address(0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff).getPrefix(it->prefixLength);
            ...
            // Create route
            src = eigrpTt->findOrCreateRoute(network, mask, eigrpTt->getRouterId(), eigrpIface, EigrpNeighbor<inet::IPv6Address>::UNSPEC_ID, &isSourceNew);
            ...
            // Notify DUAL about event
            eigrpDual->processEvent(EigrpDual<inet::IPv6Address>::INTERFACE_UP, src, EigrpNeighbor<inet::IPv6Address>::UNSPEC_ID, isSourceNew);
        }
    }
...
EigrpNetwork<inet::IPv6Address> *EigrpIpv6Pdm::addNetwork(inet::IPv6Address address, inet::IPv6Address mask)
{
...
void EigrpIpv6Pdm::sendUpdate(int destNeighbor, EigrpRoute<inet::IPv6Address> *route, EigrpRouteSource<inet::IPv6Address> *source, bool forcePoisonRev, const char *reason)
{
...
void EigrpIpv6Pdm::sendQuery(int destNeighbor, EigrpRoute<inet::IPv6Address> *route, EigrpRouteSource<inet::IPv6Address> *source, bool forcePoisonRev)
{
...
void EigrpIpv6Pdm::sendReply(EigrpRoute<inet::IPv6Address> *route, int destNeighbor, EigrpRouteSource<inet::IPv6Address> *source, bool forcePoisonRev, bool isUnreachable)
{
    EigrpMsgRoute msgRt;
    EigrpNeighbor<inet::IPv6Address> *neigh = eigrpNt->findNeighborById(destNeighbor);
 ...
 */
bool EigrpIpv6Pdm::removeRouteFromRT(EigrpRouteSource<inet::IPv6Address> *source, ANSAIPv6Route::RoutingProtocolSource *removedRtSrc)
{
    EigrpRoute<inet::IPv6Address> *route = source->getRouteInfo();
    inet::IPv6Route *rtEntry =rt->findRoute(route->getRouteAddress(), getNetmaskLength(route->getRouteMask()), source->getNextHop());
...
bool EigrpIpv6Pdm::isRTSafeForAdd(EigrpRoute<inet::IPv6Address> *route, unsigned int eigrpAd)
{
    inet::IPv6Route *routeInTable = rt->findRoute(route->getRouteAddress(), getNetmaskLength(route->getRouteMask()));
    ...
    if (ansaRoute == NULL && routeInTable->getAdminDist() == inet::IPv6Route::dUnknown)
        return false;   // Connected route has AD = 255 (dUnknown) in inet::IPv4Route
    if (routeInTable != NULL && routeInTable->getAdminDist() < eigrpAd)
        return false;   // Other inet::IPv4Route with right AD
    return true;
}

EigrpRouteSource<inet::IPv6Address> *EigrpIpv6Pdm::updateRoute(EigrpRoute<inet::IPv6Address> *route, uint64_t dmin, bool *rtableChanged, bool removeUnreach)
{
    EigrpRouteSource<inet::IPv6Address> *source = NULL, *bestSuccessor = NULL;
    inet::IPv6Route *rtEntry = NULL;
...
bool EigrpIpv6Pdm::removeOldSuccessor(EigrpRouteSource<inet::IPv6Address> *source, EigrpRoute<inet::IPv6Address> *route)
{
...
bool EigrpIpv6Pdm::installRouteToRT(EigrpRoute<inet::IPv6Address> *route, EigrpRouteSource<inet::IPv6Address> *source, uint64_t dmin, inet::IPv6Route *rtEntry)
{
...
bool EigrpIpv6Pdm::setReplyStatusTable(EigrpRoute<inet::IPv6Address> *route, EigrpRouteSource<inet::IPv6Address> *source, bool forcePoisonRev, int *neighCount, int *stubCount)
{
...
bool EigrpIpv6Pdm::hasNeighborForUpdate(EigrpRouteSource<inet::IPv6Address> *source)
{
...
void EigrpIpv6Pdm::setDelayedRemove(int neighId, EigrpRouteSource<inet::IPv6Address> *src)
{
    EigrpNeighbor<inet::IPv6Address> *neigh = eigrpNt->findNeighborById(neighId);
...
void EigrpIpv6Pdm::sendUpdateToStubs(EigrpRouteSource<inet::IPv6Address> *succ ,EigrpRouteSource<inet::IPv6Address> *oldSucc, EigrpRoute<inet::IPv6Address> *route)
{
...
bool EigrpIpv6Pdm::addNetPrefix(const inet::IPv6Address &network, const short int prefixLen, const int ifaceId)
{
    PrefixVector::iterator it;
