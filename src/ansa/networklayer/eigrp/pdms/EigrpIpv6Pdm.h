...
class EigrpIpv6Pdm : public cSimpleModule, public IEigrpModule<IPv6Address>, public IEigrpPdm<IPv6Address>, public INotifiable
{
  protected:
    ...
    bool removeRouteFromRT(EigrpRouteSource<IPv6Address> *successor, ANSAIPv6Route::RoutingProtocolSource *removedRtSrc);
    ANSAIPv6Route *createRTRoute(EigrpRouteSource<IPv6Address> *successor);
    /**
  ...
  public:
    ...
    void sendQuery(int destNeighbor, EigrpRoute<IPv6Address> *route, EigrpRouteSource<IPv6Address> *source, bool forcePoisonRev = false);
    void sendReply(EigrpRoute<IPv6Address> *route, int destNeighbor, EigrpRouteSource<IPv6Address> *source, bool forcePoisonRev = false, bool isUnreachable = false);
    EigrpRouteSource<IPv6Address> *updateRoute(EigrpRoute<IPv6Address> *route, uint64_t dmin, bool *rtableChanged, bool removeUnreach = false);
    uint64_t findRouteDMin(EigrpRoute<IPv6Address> *route) { return eigrpTt->findRouteDMin(route); }
    bool hasFeasibleSuccessor(EigrpRoute<IPv6Address> *route, uint64_t& resultDmin) { return eigrpTt->hasFeasibleSuccessor(route, resultDmin); }
    EigrpRouteSource<IPv6Address> *getBestSuccessor(EigrpRoute<IPv6Address> *route) { return eigrpTt->getBestSuccessor(route); }
    bool setReplyStatusTable(EigrpRoute<IPv6Address> *route, EigrpRouteSource<IPv6Address> *source, bool forcePoisonRev, int *neighCount, int *stubCount);
