...
cXMLElement * PIMDeviceConfigurator::GetDevice(const char *deviceType, const char *deviceId, const char *configFile){
...
cXMLElement * PIMDeviceConfigurator::GetInterface(cXMLElement *iface, cXMLElement *device){
...
void PIMDeviceConfigurator::loadPimGlobalConfig(pimSM *pimSMModule)
{
    ...
    cXMLElement* pimGlobal = GetPimGlobal(device);
    if (pimGlobal != NULL)
    {
        //RP address
...
cXMLElement * PIMDeviceConfigurator::GetPimGlobal(cXMLElement * device)
{
    if(device == NULL)
        return NULL;
