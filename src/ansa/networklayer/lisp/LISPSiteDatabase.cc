...
void LISPSiteDatabase::updateTimeout(inet::L3Address addr, std::string name) {
...
LISPEtrTimer* LISPSiteDatabase::findExpirationTimer(inet::L3Address addr, std::string name) {
    for (SiteTimeoutItem it = SiteTimeouts.begin(); it != SiteTimeouts.end(); ++it) {
        if ( (*it)->getEtrAddr()==(addr) && !name.compare((*it)->getSiteName())  )
            return *it;
    }
    return NULL;
}
...
LISPSite* LISPSiteDatabase::findSiteByAggregate(const inet::L3Address& addr) {
    LISPSite* res = NULL;
    int tmplen = 0;
    for (SiteStorageItem it = SiteDatabase.begin(); it != SiteDatabase.end(); ++it) {
...
LISPSiteRecord* LISPSiteDatabase::updateSiteEtr(LISPSite* si, inet::L3Address src, bool proxy) {
    Enter_Method_Silent();
    LISPSiteRecord* srec;
    LISPServerEntry* se;
    //Create new record ...
