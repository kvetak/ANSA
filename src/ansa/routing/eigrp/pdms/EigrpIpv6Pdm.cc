...
void EigrpIpv6Pdm::initialize(int stage)
{
    ...
    if (stage == 3)
    {
        ...
        eigrpTt = check_and_cast<EigrpIpv6TopologyTable*>(getModuleByPath("^.eigrpIpv6NeighborTable"));;
        rt = check_and_cast<IPv6RoutingTable*>(getModuleByPath(par("routingTableModule"))->getSubmodule("ipv6"));
...
void EigrpIpv6Pdm::processIfaceStateChange(ANSA_InterfaceEntry *iface)
{
...
void EigrpIpv6Pdm::processMsgFromNetwork(cMessage *msg)
{
    ...
    ANSA_InterfaceEntry::State status = ift->getInterfaceById(ifaceId)->getState();
    if(status == ANSA_InterfaceEntry::DOWN || status == ANSA_InterfaceEntry::GOING_UP )
...
void EigrpIpv6Pdm::disableInterface(ANSA_InterfaceEntry *iface, EigrpInterface *eigrpIface)
{
    EigrpTimer* hellot;
    EigrpNeighbor<IPv6Address> *neigh;
    EigrpRouteSource<IPv6Address> *source;
