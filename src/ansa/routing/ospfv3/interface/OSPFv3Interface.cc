...
void OSPFv3Interface::processLSR(OSPFv3Packet* packet, OSPFv3Neighbor* neighbor){
    ...
    if ((neighborState == OSPFv3Neighbor::EXCHANGE_STATE) ||
            (neighborState == OSPFv3Neighbor::LOADING_STATE) ||
            (neighborState == OSPFv3Neighbor::FULL_STATE))
    {
        OSPFv3LinkStateRequest *lsRequestPacket = check_and_cast<OSPFv3LinkStateRequest *>(packet);
        ...
        for (unsigned long i = 0; i < requestCount; i++) {
            OSPFv3LSRequest& request = lsRequestPacket->getRequests(i);
            LSAKeyType lsaKey;
...
void OSPFv3Interface::processLSU(OSPFv3Packet* packet, OSPFv3Neighbor* neighbor){
    EV_DEBUG << "Received LSU from " << packet->getRouterID() << "\n";

    OSPFv3LSUpdate *lsUpdatePacket = check_and_cast<OSPFv3LSUpdate *>(packet);
    ...
    if (neighbor->getState() >= OSPFv3Neighbor::EXCHANGE_STATE) {
        ...
        while (currentType >= ROUTER_LSA && currentType <= LINK_LSA){//AS_EXTERNAL_LSA) {TODO
            EV_DEBUG << "While in process LSU - currentType = " << currentType << "\n";
            unsigned int lsaCount = 0;
            switch (currentType) {
            case ROUTER_LSA:
                EV_DEBUG << "Parsing ROUTER_LSAs\n";
                lsaCount = lsUpdatePacket->getRouterLSAsArraySize();
                break;

            case NETWORK_LSA:
                EV_DEBUG << "Parsing NETWORK_LSAs\n";
...
//                OSPFv3LSAType typeStruct = static_cast<OSPFv3LSAType>(currentLSA->getHeader().getLsaType());
                uint16_t lsaType = currentLSA->getHeader().getLsaType();
                if ((lsaType != ROUTER_LSA) &&
                        (lsaType != NETWORK_LSA) &&
                        (lsaType != INTER_AREA_PREFIX_LSA) &&
                        (lsaType != INTER_AREA_ROUTER_LSA) &&
                        (lsaType != AS_EXTERNAL_LSA) &&
                        (lsaType != NSSA_LSA) &&
                        (lsaType != LINK_LSA) &&
                        (lsaType != INTRA_AREA_PREFIX_LSA))
                {
                    continue;
                }

//                FIXME area maybe nullptr
                if ((lsaType == AS_EXTERNAL_LSA) && !(area != nullptr && area->getExternalRoutingCapability())) {
                    continue;
                }
                ...
                //TODO
//                ackFlags.anyNeighborInExchangeOrLoadingState = router->hasAnyNeighborInStates(OSPFv3Neighbor::EXCHANGE_STATE | OSPFv3Neighbor::LOADING_STATE);
...
//                            this->sendLSAcknowledgement(&(currentLSA->getHeader()), neighbor->getNeighborIP());
                ...
                //LSA is not in the database or new LSA is newer
...
//                    if (currentType == AS_EXTERNAL_LSA) {
...
//                    }
                ...
                //LSA is on request list - error
                if (neighbor->isLSAOnRequestList(lsaKey)) {
                    EV_DEBUG << "Ending up here..\n";
                    neighbor->processEvent(OSPFv3Neighbor::BAD_LINK_STATE_REQUEST);
                    break;
                }
                //lsa is duplicate
                if (ackFlags.lsaIsDuplicate) {
                    //a) implied ack
                    if (neighbor->isLinkStateRequestListEmpty()){//lsaKey)) { //TODO - why with the key?
                        EV_DEBUG << "Removing from retransmission list\n";
                ...
                //database copy is more recent - simply discard without ack
                if ((lsaInDatabase->getHeader().getLsaAge() == MAX_AGE) &&
                        (lsaInDatabase->getHeader().getLsaSequenceNumber() == MAX_SEQUENCE_NUMBER))
                {
                    continue;
                }
                //Otherwise as long as the database copy has not been sent in link state update send it to the neighbor directly
                if (!neighbor->isOnTransmittedLSAList(lsaKey)) {
                    OSPFv3LSUpdate *updatePacket = this->prepareLSUHeader();
                    updatePacket = this->prepareUpdatePacket(lsaInDatabase,updatePacket);
                    if (updatePacket != nullptr) {
                        int hopLimit = (this->getType() == OSPFv3Interface::VIRTUAL_TYPE) ? VIRTUAL_LINK_TTL : 1;
...
//            else
                currentType++;
              EV_DEBUG << "Raising currentType to " << currentType << "\n";
//            currentType = static_cast<OSPFv3LSAType>(currentType + 1);
...
void OSPFv3Interface::addDelayedAcknowledgement(OSPFv3LSAHeader& lsaHeader)
{
    if (interfaceType == OSPFv3Interface::BROADCAST_TYPE) {
        ...
        for (long i = 0; i < neighborCount; i++) {
            if (this->getNeighbor(i)->getState() >= OSPFv3Neighbor::EXCHANGE_STATE) {
                delayedAcknowledgements[this->getNeighbor(i)->getNeighborIP()].push_back(lsaHeader);
            }
        }
    }
...
{
    long maxPacketSize = ((IPv6_HEADER_BYTES + OSPFV3_HEADER_LENGTH + OSPFV3_LSA_HEADER_LENGTH) > mtu) ? IPV6_DATAGRAM_LENGTH : mtu;
    ...
    for (auto delayIt = delayedAcknowledgements.begin(); delayIt != delayedAcknowledgements.end(); delayIt++)
    {
        ...
        if (ackCount > 0) {
            while (!(delayIt->second.empty())) {
                OSPFv3LSAck *ackPacket = new OSPFv3LSAck();
                long packetSize = IPv6_HEADER_BYTES + OSPFV3_HEADER_LENGTH;

                ackPacket->setType(LS_ACK);
                ackPacket->setRouterID(this->getArea()->getInstance()->getProcess()->getRouterID());
                ackPacket->setAreaID(this->getArea()->getAreaID());

                while ((!(delayIt->second.empty())) && (packetSize <= (maxPacketSize - OSPFV3_LSA_HEADER_LENGTH))) {
                ...
                ackPacket->setByteLength(packetSize - IPv6_HEADER_BYTES);
                ...
                if (interfaceType == OSPFv3Interface::BROADCAST_TYPE) {
                    ...
                    if (interfaceType == OSPFv3Interface::POINTTOPOINT_TYPE) {
                        ...
                        this->getArea()->getInstance()->getProcess()->sendPacket(ackPacket, delayIt->first, this->getIntName().c_str(), hopLimit);
                    }
                }
            }
        }
    }
    this->getArea()->getInstance()->getProcess()->setTimer(this->acknowledgementTimer, this->getAckDelay());
...
void OSPFv3Interface::removeFromAllRetransmissionLists(LSAKeyType lsaKey)
{
    long neighborCount = this->neighbors.size();
    for (long i = 0; i < neighborCount; i++) {
        this->neighbors.at(i)->removeFromRetransmissionList(lsaKey);
    }
}
