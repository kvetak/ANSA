...
void OSPFv3Area::calculateShortestPathTree(std::vector<OSPFv3RoutingTableEntry* > newTable)
{
    ...
    std::vector<OSPFv3RouterNode* > routerNodes;
    std::vector<OSPFv3NetworkNode* > networkNodes;
    unsigned long i, j, k;
    unsigned long lsaCount;

    if (spfTreeRoot == nullptr) {
        EV_DEBUG << "SPFTreeRoot is empty!\n";
        return;
    }

    lsaCount = this->getRouterLSACount();//routerLSAs.size();
    for (i = 0; i < lsaCount; i++) {
        routerNodes.push_back(new OSPFv3RouterNode(*(this->getRouterLSA(i))));
        //routerLSAs[i]->clearNextHops();
    }

    lsaCount = this->getNetworkLSACount();
    for (i = 0; i < lsaCount; i++) {
        networkNodes.push_back(new OSPFv3NetworkNode(*(this->getNetworkLSA(i))));
        //networkLSAs[i]->clearNextHops();
    }
    spfTreeRoot->setDistance(0);
    treeVertices.push_back(spfTreeRoot);
    justAddedVertex = spfTreeRoot;
    ...
    do{
        ...
        if (vertexType == ROUTER_LSA) {
            EV_DEBUG << "SPFTREE: Parsing ROUTER LSA\n";
            OSPFv3RouterNode *routerVertex = check_and_cast<OSPFv3RouterNode *>(justAddedVertex);
            if (routerVertex->getVBit()) {    // (2)
                transitCapability = true;
            }

            unsigned int linkCount = routerVertex->getRoutersArraySize();
            for (i = 0; i < linkCount; i++) {
                OSPFv3RouterLSABody& router = routerVertex->getRouters(i);//Link& link = routerVertex->getLinks(i);
                OSPFv3RouterLSAType type = static_cast<OSPFv3RouterLSAType>(router.type);//LinkType linkType = static_cast<LinkType>(link.getType());
                OSPFv3LSA *joiningVertex=nullptr;
                OSPFv3LSAFunctionCode joiningVertexType;

                LSAKeyType lsaKey;

                if (type == TRANSIT_NETWORK) {
                    EV_DEBUG << "SPFTREE: type is transit\n";
                    IPv4Address designatedRtr = this->getInterfaceById(router.interfaceID)->getDesignatedID();

                    for(int m=0; m<this->getNetworkLSACount();m++){
                        OSPFv3LSA* lsa = this->getNetworkLSA(m);
                        if(lsa->getHeader().getAdvertisingRouter() == designatedRtr) {
                            joiningVertex = lsa;
                            EV_DEBUG << "SPFTREE: Found Network LSA\n";
                            break;
                        }
                    }
                    ...
                    EV_DEBUG << "SPFTREE: type is router - P2P TODO\n";
                    for(int m=0; m<this->getRouterLSACount(); m++) {
                        OSPFv3LSA* lsa = this->getRouterLSA(m);
                        if(lsa->getHeader().getAdvertisingRouter() == router.neighborRouterID) {
                            joiningVertex = lsa;
                            EV_DEBUG << "SPFTREE: Found Router LSA\n";
                            break;
                        }
                    }
                ...
                EV_DEBUG << "SPFTREE: treesize is " << treeSize << "\n";

                for (j = 0; j < treeSize; j++) {
                    EV_DEBUG << "SPFTREE: j = " << j << "\n";
                ...
                if (alreadyOnTree) {    // (2) (c)
                    EV_DEBUG << "SPFTREE: Current vertex is already on the tree\n";
                    continue;
                }

                unsigned long linkStateCost = routerVertex->getDistance() + router.metric;
                ...
                if (candidate != nullptr) {    // (2) (d)
                    EV_DEBUG << "SPFTREE: Choosing candidate\n";
                    ...
                    if (joiningVertexType == ROUTER_LSA) {
                        EV_DEBUG << "SPFTREE: Joining vertex is router, cast is successful\n";
                        OSPFv3RouterNode *joiningRouterVertex = check_and_cast<OSPFv3RouterNode *>(joiningVertex);
                        ...
                        OSPFv3NetworkLSA& netVert = dynamic_cast<OSPFv3NetworkLSA&>(*joiningVertex);
                        OSPFv3NetworkNode *joiningNetworkVertex = new OSPFv3NetworkNode(netVert);
                        EV_DEBUG << "SPFTREE: Joining vertex is network, cast is successful\n";
...
{
    EV_DEBUG << "HAS LINK FUNCTION\n";
    unsigned int i;

    OSPFv3RouterLSA *fromRouterLSA = dynamic_cast<OSPFv3RouterLSA *>(fromLSA);
    if (fromRouterLSA != nullptr) {
        EV_DEBUG << "\tFrom Router is Router LSA\n";
        unsigned int linkCount = fromRouterLSA->getRoutersArraySize();
        OSPFv3RouterLSA *toRouterLSA = dynamic_cast<OSPFv3RouterLSA *>(toLSA);
        if (toRouterLSA != nullptr) {
            EV_DEBUG << "\tTo Router is Router LSA\n";
            for (i = 0; i < linkCount; i++) {
                OSPFv3RouterLSABody& router = fromRouterLSA->getRouters(i);//Link& link = fromRouterLSA->getLinks(i);
                OSPFv3RouterLSAType type = static_cast<OSPFv3RouterLSAType>(router.type);//LinkType linkType = static_cast<LinkType>(link.getType());

                if (((type == POINT_TO_POINT) ||
                     (type == VIRTUAL_LINK)) &&
                    (router.neighborRouterID == toRouterLSA->getHeader().getAdvertisingRouter())) //TODO check
                {
                    return true;
                }
            }
            ...
            OSPFv3NetworkLSA *toNetworkLSA = dynamic_cast<OSPFv3NetworkLSA *>(toLSA);
            if (toNetworkLSA != nullptr) {
                EV_DEBUG << "\tTo Router is Network LSA\n";
                for (i = 0; i < linkCount; i++) {
                    OSPFv3RouterLSABody& router = fromRouterLSA->getRouters(i);//Link& link = fromRouterLSA->getLinks(i);

                    if ((router.type == TRANSIT_NETWORK) &&
                        (router.neighborRouterID == toNetworkLSA->getHeader().getAdvertisingRouter()))
                    {
                        EV_DEBUG << "\tReturning TRUE\n";
                        return true;
                    }
                }
            }
        ...
        EV_DEBUG << "\tFrom Router is null\n";
        OSPFv3NetworkLSA *fromNetworkLSA = dynamic_cast<OSPFv3NetworkLSA*>(fromLSA);
        if (fromNetworkLSA != nullptr) {
            EV_DEBUG << "\tFrom Router is Network LSA\n";
