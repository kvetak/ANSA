...
static std::ostream & operator<<(std::ostream & os, const AnsaUDP::SockDescList& list)
{
    for (AnsaUDP::SockDescList::const_iterator i=list.begin(); i!=list.end(); ++i)
        os << "sockId=" << (*i)->sockId << " ";
    return os;
}
...
void AnsaUDP::bind(int gateIndex, AnsaUDPControlInfo *ctrl)
{
...
void AnsaUDP::connect(int sockId, IPvXAddress addr, int port)
{
...
void AnsaUDP::unbind(int sockId)
{
...
short AnsaUDP::getEphemeralPort()
{
...
void AnsaUDP::handleMessage(cMessage *msg)
{
...
void AnsaUDP::updateDisplayString()
{
    char buf[80];
...
bool AnsaUDP::matchesSocket(SockDesc *sd, UDPPacket *udp, IPControlInfo *ipCtrl)
{
    // IPv4 version
...
bool AnsaUDP::matchesSocket(SockDesc *sd, UDPPacket *udp, IPv6ControlInfo *ipCtrl)
{
    // IPv6 version
    if (sd->remotePort!=0 && sd->remotePort!=udp->getSourcePort())
        return false;
...
bool AnsaUDP::matchesSocket(SockDesc *sd, const IPvXAddress& localAddr, const IPvXAddress& remoteAddr, short remotePort)
{
...
void AnsaUDP::sendUp(cPacket *payload, UDPPacket *udpHeader, IPControlInfo *ipCtrl, SockDesc *sd)
{
...
void AnsaUDP::sendUp(cPacket *payload, UDPPacket *udpHeader, IPv6ControlInfo *ipCtrl, SockDesc *sd)
{
...
void AnsaUDP::processUndeliverablePacket(UDPPacket *udpPacket, cPolymorphic *ctrl)
{
    numDroppedWrongPort++;
...
void AnsaUDP::processICMPError(cPacket *msg)
{
...
void AnsaUDP::sendUpErrorNotification(SockDesc *sd, int msgkind, const IPvXAddress& localAddr, const IPvXAddress& remoteAddr, short remotePort)
{
...
void AnsaUDP::processUDPPacket(UDPPacket *udpPacket)
{
...
void AnsaUDP::processMsgFromApp(cPacket *appData)
{
...
UDPPacket *AnsaUDP::createUDPPacket(const char *name)
{
    return new UDPPacket(name);
}

void AnsaUDP::processCommandFromApp(cMessage *msg)
{
