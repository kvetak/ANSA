...
void AnsaOspf6::NeighborStateExchange::ProcessEvent(AnsaOspf6::Neighbor* neighbor,
   ...
   if (event == AnsaOspf6::Neighbor::ExchangeDone){
      if (neighbor->IsLinkStateRequestListEmpty()){
         MessageHandler* messageHandler =
               neighbor->GetInterface()->GetArea()->GetRouter()->GetMessageHandler();
         messageHandler->StartTimer(neighbor->GetDDRetransmissionTimer(),
               neighbor->GetRouterDeadInterval());
         ...
         messageHandler->StartTimer(neighbor->GetDDRetransmissionTimer(),
               neighbor->GetRouterDeadInterval());
         ChangeState(neighbor, new AnsaOspf6::NeighborStateLoading, this);
      }
   }
