        ...
        if (len(stlpce) == 6) and (stlpce[1][0:8] == "appData-"):
            if (stlpce[0] == "Send:"):
                ...
                if (started == 1) and (index + 1 < len(ev_starts)) and (float(ev_starts[index + 1]) <=  float(stlpce[5])):
                    ev_endlimit.append(float(event))
                    index = index + 1
            ...
            if (stlpce[0] == "Arrive:"):
                ...
                if (started == 1):
                    ev_endlimit.append(float(stlpce[5]))
                    started = 0
                    index = index + 1
                    if (index < len(ev_starts)):
                        event = ev_starts[index]
                        ...
                        event = 99999999
if (started == 1):
    ev_endlimit.append(float(750.0))
subor.close()

subor=file('omnetpp.out','r')

cas=0.0
aktualny=0.0

index = 0;
event_st = ev_starts[index]
event_lim = ev_endlimit[index]

for riadok in subor:
    stlpce= riadok.split()
    if (len(stlpce) > 3) and (stlpce[0] == "**"):
        aktualny=float(stlpce[3][2:])
        if (aktualny > event_lim):
            if (cas > 0.0):
                convergence_time.append(cas - event_st)
            else:
                convergence_time.append(-1.0)
            index = index + 1
            cas = -1.0
            if (index < len(ev_endlimit)):
                event_st = ev_starts[index]
                event_lim = ev_endlimit[index]
            else:
                event_st = 999999999.9
                event_lim = 999999999.9
    if (riadok == 'Routing table was rebuilt.\n') and (aktualny > event_st):
        cas=aktualny
subor.close()
...
if (len(sys.argv)>1) and (sys.argv[1] == "delay"):
    ...
    for number in rangelist:
        if (sent_time["appData-" + str(number)] > float(event) + 20.0):
            if (sent_time["appData-" + str(number)] < float(event) + 21.0):
